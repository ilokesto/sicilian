...
*  Sicilian is based on global state, which means the playDragon function must be declared outside of components. As a result, it was challenging to use Sicilian in cases requiring local state—such as comment creation or editing features. However, with the addition of the **useDragon** hook in version 2.1.3 of Sicilian, this limitation has been resolved. The useDragon hook can be called within components, allowing you to leverage all of Sicilian's features while functioning like local state.
